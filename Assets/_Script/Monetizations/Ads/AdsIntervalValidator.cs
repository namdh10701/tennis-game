            ...
            switch (type)
            {
                ...
                case AdType.INTER:
                    ...
                    if (timeShowInterAdsBetween_Inter_InterToNow.TotalSeconds > 0
                        && timeShowInterAdsBetween_Inter_InterToNow.TotalSeconds < AdsController.Instance.INTERVAL_INTER_INTER)
                    {
                        ret = false;
                    }
                    ...
                    if (timeShowInterAdsBetween_OpenAds_Inter.TotalSeconds > 0
                        && timeShowInterAdsBetween_OpenAds_Inter.TotalSeconds < AdsController.Instance.INTERVAL_OPEN_INTER)
                    {
                        ret = false;
                    }
                    break;
